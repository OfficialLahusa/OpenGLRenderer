...
ReflectMaterial::ReflectMaterial(PointLight& light, Camera& cam, Texture& tex)
	: m_shader("res/shaders/phong_old.shader")
	, m_light(light)
	, m_cam(cam)
	, m_tex(&tex)
...
void ReflectMaterial::loadLight(PointLight light) {
	m_shader.Bind();
	m_shader.setUniform3f("u_LightPos", light.getPosition());
	m_shader.setUniform3f("u_SpecularColor", light.getColor());
	m_light = light;
...
void ReflectMaterial::loadCamera(Camera& cam) {
	m_shader.Bind();
	m_shader.setUniformMat4f("u_ViewMatrix", cam.GetViewMatrix(), false);
	m_shader.setUniformMat4f("u_ProjectionMatrix", cam.GetProjectionMatrix(), false);
	m_cam = cam;
...
void ReflectMaterial::loadModel(glm::mat4 model) {
	m_shader.Bind();
	m_shader.setUniformMat4f("u_ModelMatrix", model, false);
	m_model = model;
}

void ReflectMaterial::setTexture(Texture& tex) {
	m_tex = &tex;
}
